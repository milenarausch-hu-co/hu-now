...
function UrgencyBadge({ urgencia }) {
  const cls = URGENCIA_COLORS[urgencia] ?? 'bg-slate-400 text-white';
...
function BackButton({ onClick, label = 'Volver' }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-2 text-sm text-slate-700 font-bold hover:text-slate-900 transition mb-4 bg-slate-100 px-4 py-2 rounded-full"
    >
...
function ListView({ reportes, loading, onVerDetalle }) {
  return (
    <div className="flex flex-col px-4 py-4">
      <h2 className="text-2xl font-bold text-slate-900 mb-4">Mis reportes</h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-slate-800 font-bold text-lg">Todavía no enviaste ningún reporte.</p>
          <p className="text-sm text-slate-500 mt-2">Usá la tab "Reportar" para describir un incidente.</p>
        </div>
      ) : (
          ...
          {reportes.map(r => (
            <div key={r.id} className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
              ...
              <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-2">
                <div>
                  <p className="font-mono text-xs font-bold text-slate-700 bg-slate-200 px-2 py-1 rounded inline-block">{r.id}</p>
                  <p className="text-xs text-slate-500 mt-1">{fmtDate(r.fecha_generacion_reporte)}</p>
                </div>
              ...
              <div className="px-4 py-4">
                <p className="text-base font-semibold text-slate-900 mb-3">{r.descripcion_corta ?? '—'}</p>

                {/* Estado message */}
                <p className="text-sm text-slate-700 bg-slate-50 rounded-xl px-4 py-3 mb-3 font-medium border border-slate-100">
                  {getMensajeEstado(r)}
                </p>

                {/* Tiempo estimado */}
                {TIEMPO_URGENCIA[r.urgencia] && (
                  <p className="text-xs text-slate-500 font-medium">{TIEMPO_URGENCIA[r.urgencia]}</p>
                )}
              </div>
              ...
              <div className="px-4 pb-4">
                <button
                  ...
                  className="w-full py-3 bg-cyan-500 text-slate-900 rounded-full text-sm font-bold hover:bg-cyan-400 active:scale-[0.98] transition-all shadow"
                >
...
function HistorialTimeline({ historial }) {
  if (!historial || historial.length === 0) {
    return <p className="text-sm text-slate-500 italic">Sin historial disponible.</p>;
  }
      ...
      {historial.map((entry, i) => (
        <div key={i} className="flex gap-3">
          ...
          <div className="flex flex-col items-center">
            <div className={`w-4 h-4 rounded-full flex-shrink-0 mt-0.5 ${estadoColors[entry.estado] ?? 'bg-slate-400'}`} />
            {i < historial.length - 1 && <div className="w-0.5 flex-1 bg-slate-200 my-1" />}
          </div>
          {/* Content */}
          <div className={`${i < historial.length - 1 ? 'pb-4' : 'pb-1'}`}>
            <p className="text-sm font-bold text-slate-800">{entry.estado}</p>
            <p className="text-xs text-slate-500">{fmtDateShort(entry.fecha)}</p>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="flex flex-col px-4 py-4 gap-4">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="bg-slate-900 px-5 py-4 flex items-center justify-between gap-2">
          <div>
            <p className="font-mono font-black text-cyan-400 text-xl">{reporte.id}</p>
            <p className="text-xs text-slate-400 mt-0.5">{fmtDate(reporte.fecha_generacion_reporte)}</p>
          </div>
          <div className="flex flex-col items-end gap-2">
        ...
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50">
          <p className="text-sm text-slate-800 font-medium">{getMensajeEstado(reporte)}</p>
          {TIEMPO_URGENCIA[reporte.urgencia] && (
            <p className="text-xs text-slate-500 mt-1">{TIEMPO_URGENCIA[reporte.urgencia]}</p>
          )}
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5">
        <h3 className="text-base font-bold text-slate-900 mb-4">Historial de estados</h3>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 flex items-center gap-3">
          ...
          <h3 className="text-base font-bold text-slate-900">Mientras se resuelve tu reporte:</h3>
        </div>
            ...
            <ul className="space-y-3">
              {recomendaciones.map((rec, i) => (
                <li key={i} className="flex gap-3 text-sm text-slate-700">
                  <span className="w-7 h-7 rounded-lg bg-slate-900 text-cyan-400 flex items-center justify-center text-xs font-bold flex-shrink-0">{i + 1}</span>
                  <span className="pt-1">{rec}</span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-sm text-slate-500 italic">No se pudieron cargar las recomendaciones.</p>
          )}
        </div>
      </div>
    </div>
